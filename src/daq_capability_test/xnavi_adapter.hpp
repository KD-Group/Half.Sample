...
inline AdapterResult<OperationInfo> validate_xnavi_function_table(const XNaviFunctionTableView* table) {
    ...
    if (!table || table->version != EXPECTED_DAQNAVI_TABLE_VERSION ||
        ...
        const std::string actual =
            table ? std::to_string(table->version) + "." + std::to_string(table->revision) : "null";
...
inline std::vector<std::vector<double>> deinterleave_xnavi_samples(const std::vector<double>& interleaved,
...
inline AdapterResult<OperationInfo> validate_xnavi_config_readback(const OperationInfo& requested,
...
inline std::vector<std::string> xnavi_required_runtime_exports() { return {"AdxDaqNaviLibInitialize"}; }
