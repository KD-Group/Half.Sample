...
std::string to_json(const CommandResult& result) {
    std::ostringstream output;
    output << "{\"result\":\"" << status_name(result.status) << "\",\"code\":\"" << escape_json(result.code)
           << "\",\"message\":\"" << escape_json(result.message) << "\",\"evidence\":{";

    bool first = true;
    for (std::map<std::string, std::string>::const_iterator it = result.evidence.begin(); it != result.evidence.end();
         ++it) {
        if (!first) {
            output << ',';
        }
...
std::string json_string(const std::string& value) { return std::string("\"") + escape_json(value) + "\""; }
