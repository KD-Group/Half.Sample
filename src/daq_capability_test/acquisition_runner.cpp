...
namespace {
std::string number(double value) { std::ostringstream out; out << value; return out.str(); }
std::string number(unsigned int value) { std::ostringstream out; out << value; return out.str(); }

CommandResult result(Status status, ExitCategory category, const std::string& code, const std::string& message = "")
{
    CommandResult r; r.status = status; r.exit_category = category; r.code = code; r.message = message; return r;
}

template <typename T>
CommandResult adapter_failure(const AdapterResult<T>& value)
{
    ...
    if (!value.stage.empty()) r.evidence["stage"] = value.stage;
    if (!value.driver_error.empty()) r.evidence["driver_error"] = value.driver_error;
    return r;
}
...
public:
    ...
    void stop_now() noexcept { if (armed_) { adapter_.stop(); armed_ = false; } }
private:
    DaqAdapter& adapter_;
    bool armed_;
};
...
{
    if (!c.device.has_value()) field = "device";
    else if (!c.sample_channel.has_value()) field = "sample_channel";
    else if (!c.value_range.has_value()) field = "value_range";
    else if (!c.sample_rate_hz.has_value() || c.sample_rate_hz.value() <= 0.0) field = "sample_rate_hz";
    else if (!c.points_per_channel.has_value() || c.points_per_channel.value() <= 0) field = "points_per_channel";
    else if (!c.repeat_count.has_value() || c.repeat_count.value() <= 0) field = "repeat_count";
    else if (!c.timeout_seconds.has_value() || c.timeout_seconds.value() <= 0.0) field = "timeout_seconds";
    else if (c.reference_channel.has_value() && c.reference_channel.value() == c.sample_channel.value()) field = "reference_channel";
    else if (c.signal_frequency_hz.has_value() && c.signal_frequency_hz.value() <= 0.0) field = "signal_frequency_hz";
    else if (c.min_complete_cycles.has_value() && c.min_complete_cycles.value() < 0) field = "min_complete_cycles";
    else if (c.min_signal_span_v.has_value() && c.min_signal_span_v.value() < 0.0) field = "min_signal_span_v";
    return field.empty();
}

CommandResult validate_data(const AcquisitionData& data, const AcquisitionRequest& request, const MatrixCase& c)
{
    if (data.timed_out) return result(Status::Fail, ExitCategory::Driver, "TIMEOUT");
    if (data.overrun) return result(Status::Fail, ExitCategory::Driver, "OVERRUN");
    if (data.cache_overflow) return result(Status::Fail, ExitCategory::Driver, "CACHE_OVERFLOW");
    if (!std::isfinite(data.actual_sample_rate_hz) || data.actual_sample_rate_hz <= 0.0) {
        ...
        r.evidence["field"] = "actual_sample_rate_hz"; return r;
    }
    if (!std::isfinite(data.duration_seconds) || data.duration_seconds <= 0.0) {
        CommandResult r = result(Status::Fail, ExitCategory::ValidationFailed, "INVALID_ACQUISITION_DATA");
        r.evidence["field"] = "duration_seconds"; return r;
    }
    if(!std::isfinite(data.trigger_wait_seconds)||data.trigger_wait_seconds<0||!std::isfinite(data.wall_elapsed_seconds)||data.wall_elapsed_seconds<0){CommandResult r=result(Status::Fail,ExitCategory::ValidationFailed,"INVALID_ACQUISITION_DATA","Trigger wait and wall elapsed time must be finite and non-negative");r.evidence["field"]="trigger_wait_seconds";return r;}
    for (unsigned int channel=0; channel<data.channels.size(); ++channel) {
        for (unsigned int sample=0; sample<data.channels[channel].samples.size(); ++sample) {
            if (!std::isfinite(data.channels[channel].samples[sample])) {
                CommandResult r = result(Status::Fail, ExitCategory::ValidationFailed, "INVALID_ACQUISITION_DATA");
                r.evidence["field"] = "sample"; r.evidence["channel"] = number(channel);
                r.evidence["sample_index"] = number(sample); return r;
            }
        }
    }
    ...
    for (unsigned int i = 0; i < expected_channels; ++i) {
        if (data.channels[i].samples.size() != request.points_per_channel) {
            ...
            r.evidence["actual_points_per_channel"] = number(static_cast<unsigned int>(data.channels[i].samples.size()));
    ...
    if (rate_error > 1e-9) return result(Status::Fail, ExitCategory::ValidationFailed, "CONFIG_READBACK_MISMATCH");
    ...
    if (c.signal_frequency_hz.has_value()) passed.evidence["complete_cycles"] = number(static_cast<unsigned int>(complete_cycles));
    if (c.min_signal_span_v.has_value()) passed.evidence["signal_span_v"] = number(signal_span);
    return passed;
}

CommandResult unsupported(const std::string& code)
{ return result(Status::Skip, ExitCategory::Unsupported, code); }
...
{
    if (!capabilities.supports_acquisition) return unsupported("ACQUISITION_UNSUPPORTED");
    if (request.channels.size() > capabilities.max_channels) return unsupported("CHANNEL_COUNT_UNSUPPORTED");
    ...
    if ((capabilities.max_points_per_channel > 0 && request.points_per_channel > capabilities.max_points_per_channel) ||
        (capabilities.buffer_capacity > 0 && request.points_per_channel * request.channels.size() > capabilities.buffer_capacity))
        return unsupported("POINT_COUNT_UNSUPPORTED");
    if (!capabilities.supported_ranges.empty()) {bool matched=false;for(size_t i=0;i<capabilities.supported_ranges.size();++i)if(equivalent_voltage_range(capabilities.supported_ranges[i],request.value_range))matched=true;if(!matched)return unsupported("RANGE_UNSUPPORTED");}
...
{
    Matrix matrix; matrix.cases.push_back(c);
    CommandResult preflight_result = preflight(matrix);
    if (preflight_result.status != Status::Pass) return preflight_result;
    ...
    if (!valid(c, invalid_field)) {
        ...
        r.evidence["field"] = invalid_field; return r;
    }

    AcquisitionRequest request;
    request.device = c.device.value(); request.channels.push_back(c.sample_channel.value());
    if (c.reference_channel.has_value()) request.channels.push_back(c.reference_channel.value());
    request.value_range = c.value_range.value(); request.sample_rate_hz = c.sample_rate_hz.value();
    ...
    for (int repetition = 1; repetition <= c.repeat_count.value(); ++repetition) {
        ...
        if (configured_successfully) {
            try {
                ...
                if(current.status==Status::Pass && observer) current=observer(static_cast<unsigned int>(repetition-1),acquired.value);
        ...
        stop.stop_now();
        if (current.status == Status::Pass) { ++passed; last_success = current; continue; }
        const std::string stage = current.evidence.find("stage") == current.evidence.end() ?
            "configure" : current.evidence.at("stage");
        if (failure_count == 0) first_failure = current;
        ...
        ++failure_count;
        if (current.status == Status::Skip) break;
    }
    if (failure_count > 0) {
    ...
    success.code = (c.signal_frequency_hz.has_value() || c.min_signal_span_v.has_value()) ?
        "LOW_RATE_WINDOW_VALID" : (request.channels.size() > 1 ? "DUAL_CHANNEL_REFERENCE_READY" : "ACQUISITION_STABLE");
    success.evidence["passed_repetitions"] = number(passed);
    return success;
}

CommandResult validate_acquisition_data(const AcquisitionData& data,const AcquisitionRequest& request,const MatrixCase* matrix_case)
{
    MatrixCase empty;return validate_data(data,request,matrix_case?*matrix_case:empty);
}
