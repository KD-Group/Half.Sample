...
template <typename T> class MatrixValue {
  public:
    ...
    static MatrixValue required() {
        MatrixValue result;
        result.state_ = State::Required;
        return result;
    }
    static MatrixValue from_value(const T& value) {
        MatrixValue result;
    ...
    const T& value() const {
        if (state_ == State::Empty)
            throw std::logic_error("MatrixValue is Empty");
        if (state_ == State::Required)
            throw std::logic_error("MatrixValue is Required");
        return value_;
    }
...
struct MatrixCase {
    ...
    MatrixValue<double> sample_rate_hz, timeout_seconds, signal_frequency_hz, min_signal_span_v, max_edge_jitter_us,
        min_overlap_percent, max_overlap_error_v, max_response_drift_v, max_boundary_jump_v,
        max_delay_position_error_us, reference_duty_cycle_percent, max_duty_cycle_error_percent;
