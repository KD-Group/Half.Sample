...
class XNaviDemoIntegrationTest(unittest.TestCase):
    def test_normative_range_runs_single_channel_case_and_rejects_ambiguous_range(self):
        ...
        cli = ROOT / "cpp_build/daq_capability_test_xnavi.exe"
        if not cli.is_file():
            self.skipTest("XNavi CLI has not been built")
        matrix_lines = (
    ...
    def test_demo_layout_oracle_is_repeatable_and_restores_ranges(self):
        ...
        for repetition in range(2):
            ...
            self.assertIn("real_PCI_layout_requires_hardware_confirmation=true", output)
