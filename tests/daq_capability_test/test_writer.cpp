...
std::wstring wide(const std::string& value) {
    ...
    assert(MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, value.data(), static_cast<int>(value.size()), &result[0],
                               count) == count);
    return result;
}
bool exists(const std::string& path) { return GetFileAttributesW(wide(path).c_str()) != INVALID_FILE_ATTRIBUTES; }
void make_dir(const std::string& path) {
    assert(CreateDirectoryW(wide(path).c_str(), NULL) || GetLastError() == ERROR_ALREADY_EXISTS);
}
...
struct TempDir {
    ...
    TempDir()
        : path(join("cpp_build",
                    "writer-" + std::to_string(process_id()) + "-" + std::to_string(next_id.fetch_add(1)))) {
        make_dir("cpp_build");
        make_dir(path);
    }
...
std::string bytes(const std::string& path) {
    ...
    char buffer[4096];
    DWORD count = 0;
    while (ReadFile(file, buffer, sizeof(buffer), &count, NULL) && count)
...
#ifdef _WIN32
    WIN32_FIND_DATAW data;
    HANDLE find = FindFirstFileW((wide(path) + L"/*").c_str(), &data);
    if (find != INVALID_HANDLE_VALUE) {
        do {
            if (data.cFileName[0] != L'.') {
                int n = WideCharToMultiByte(CP_UTF8, 0, data.cFileName, -1, NULL, 0, NULL, NULL);
                std::string s(static_cast<size_t>(n), '\0');
                WideCharToMultiByte(CP_UTF8, 0, data.cFileName, -1, &s[0], n, NULL, NULL);
...
CapabilityRecord capability() {
    return {"dev\t1", 2, 0.1, 1000.25, 2000, 4096, true, 2, "digital\nD0", "start\\stop", 0.125, 1.5};
}
SummaryRecord summary() {
    return {"rate\tname", 1,   1000.25, 999.5,   2,     100,   99,    0.1,    0.10005, true,   "D0\n", "rising",
            "start\\x",   0.0, 1.0,     1.10005, false, false, false, "E\t1", "read",  "PASS", "OK",   "note\nline"};
}
...
class FakeFileOperations : public FileOperations {
  public:
    ...
    FsResult create_directory(const std::string& path) override {
        ...
        if (fail == "mkdir" || path == fail_create_path)
            return FsResult(false, false, 5, "mkdir failed");
        if (::exists(path))
    ...
    FsResult exists(const std::string& path) override {
        return fail == "exists" ? FsResult(false, false, 5, "exists failed") : FsResult(true, ::exists(path), 0, "");
    }
    FsResult write_exclusive(const std::string& path, const std::string& data) override {
        if (fail == "write" || fail == "write_remove")
            return FsResult(false, false, 5, "write failed", "write");
        if (fail == "flush") {
            put(path, data);
            return FsResult(false, false, 6, "flush failed", "flush");
        }
        if (fail == "close") {
            put(path, data);
            return FsResult(false, false, 7, "close failed", "close");
        }
        put(path, data);
        return FsResult();
    }
    FsResult publish_no_replace(const std::string& temporary, const std::string& final) override {
        if (publish_creates_final)
            put(final, bytes(temporary));
        if (fail == "publish" || fail == "publish_remove")
            return FsResult(false, false, 8, "publish failed", "publish");
        return FsResult(false, false, 5, "unused");
    }
    FsResult remove(const std::string& path) override {
        ++remove_calls;
        if (fail == "remove" || fail == "write_remove" || fail == "publish_remove")
            return FsResult(false, false, 9, "remove failed", "remove");
#ifdef _WIN32
  ...
  private:
    FsResult answer(const std::string& operation) {
        return fail == operation ? FsResult(false, false, 5, operation + " failed") : FsResult();
    }
};
...
void test_unicode_root_and_exact_schemas() {
    TempDir temp;
    const std::string root = join(temp.path, "\xe4\xb8\xad\xe6\x96\x87-root");
    make_dir(root);
    ResultWriter writer(root, [] { return "20260711-123456"; });
    assert(writer.create_run_directory().status == Status::Pass);
    ...
    std::istringstream ci(cap), si(sum);
    std::string ch, cv, sh, sv;
    std::getline(ci, ch);
    std::getline(ci, cv);
    std::getline(si, sh);
    std::getline(si, sv);
    ...
    assert(cv.find("dev\\t1") == 0 && cv.find("0.10000000000000001") != std::string::npos &&
           cv.find("digital\\nD0") != std::string::npos);
...
void test_directory_plans_have_platform_specific_root_semantics() {
    ...
    const Example examples[] = {
        ...
        {"relative\\alpha/beta", detail::PathFlavor::Windows, {"relative", "relative/alpha", "relative/alpha/beta"}}};
    for (size_t i = 0; i < sizeof(examples) / sizeof(examples[0]); ++i) {
        RecordingFileOperations operations;
        std::string failed;
...
void test_root_only_paths_are_checked_but_not_created() {
    for (const std::pair<std::string, detail::PathFlavor>& example :
        ...
        assert(operations.exists_calls ==
               std::vector<std::string>(1, detail::plan_directories(example.first, example.second).root_path));
    }
}
...
void test_unsafe_or_incomplete_paths_are_rejected_before_create() {
    ...
    const Example examples[] = {
        {"new/../root", detail::PathFlavor::Posix},         {"new/./root", detail::PathFlavor::Posix},
        {"C:\\new\\..\\root", detail::PathFlavor::Windows}, {"C:", detail::PathFlavor::Windows},
        {"\\\\server", detail::PathFlavor::Windows},        {"\\\\server\\", detail::PathFlavor::Windows}};
    for (size_t i = 0; i < sizeof(examples) / sizeof(examples[0]); ++i) {
        RecordingFileOperations operations;
        std::string failed;
        const FsResult result = detail::ensure_directory_path(operations, examples[i].path, examples[i].flavor, failed);
...
void test_config_snapshot_is_byte_exact_and_existing_final_is_preserved() {
    TempDir temp;
    ResultWriter writer(temp.path, [] { return "snapshot"; });
    assert(writer.create_run_directory().status == Status::Pass);
    const std::string source = join(temp.path, "source.ini");
...
void test_raw_channel_length_mismatch_leaves_no_output() {
    TempDir temp;
    ResultWriter writer(temp.path, [] { return "raw-length-mismatch"; });
    assert(writer.create_run_directory().status == Status::Pass);
    ...
    for (size_t i = 0; i < names.size(); ++i)
        assert(names[i].find(".tmp.") == std::string::npos);
}

void test_instant_ai_raw_schema_and_validation() {
    TempDir temp;
    ResultWriter writer(temp.path, [] { return "instant-ai"; });
    assert(writer.create_run_directory().status == Status::Pass);
    InstantAiRead first;
    first.read_index = 0;
    first.elapsed_seconds = 0.001;
    first.call_duration_us = 80;
    first.interval_us = 0;
    first.values = {1.25, 2.5};
    InstantAiRead second;
    second.read_index = 1;
    second.elapsed_seconds = 0.0012;
    second.call_duration_us = 70;
    second.interval_us = 200;
    second.values = {1.5, 2.75};
    assert(writer.write_instant_ai_raw({1, 2}, {first, second}).status == Status::Pass);
    assert(bytes(join(join(writer.run_directory(), "raw"), "instant_ai_polling.tsv")) ==
           "read_index\telapsed_seconds\tcall_duration_us\tinterval_us\tchannel_1\tchannel_2\n"
           "0\t0.001\t80\t0\t1.25\t2.5\n"
           "1\t0.0011999999999999999\t70\t200\t1.5\t2.75\n");

    TempDir bad_temp;
    ResultWriter bad_writer(bad_temp.path, [] { return "instant-ai-bad"; });
    assert(bad_writer.create_run_directory().status == Status::Pass);
    first.values = {1.0};
    const CommandResult bad = bad_writer.write_instant_ai_raw({1, 2}, {first});
    assert(bad.status == Status::Fail && bad.code == "INVALID_INSTANT_AI_DATA");
    assert(!exists(join(join(bad_writer.run_directory(), "raw"), "instant_ai_polling.tsv")));
}

void test_all_nonfinite_doubles_are_rejected() {
    typedef double CapabilityRecord::* CapabilityDouble;
    const CapabilityDouble capability_fields[] = {
        &CapabilityRecord::min_sample_rate_hz, &CapabilityRecord::max_sample_rate_hz,
        &CapabilityRecord::trigger_delay_min, &CapabilityRecord::trigger_delay_max};
    for (size_t i = 0; i < sizeof(capability_fields) / sizeof(capability_fields[0]); ++i) {
        TempDir temp;
        ResultWriter writer(temp.path, [i] { return "cap-" + std::to_string(i); });
        assert(writer.create_run_directory().status == Status::Pass);
        CapabilityRecord c = capability();
        c.*capability_fields[i] = std::numeric_limits<double>::infinity();
    ...
    const SummaryDouble summary_fields[] = {&SummaryRecord::requested_sample_rate_hz,
                                            &SummaryRecord::actual_sample_rate_hz,
                                            &SummaryRecord::expected_duration_seconds,
                                            &SummaryRecord::measured_duration_seconds,
                                            &SummaryRecord::trigger_delay,
                                            &SummaryRecord::trigger_wait_seconds,
                                            &SummaryRecord::wall_elapsed_seconds};
    for (size_t i = 0; i < sizeof(summary_fields) / sizeof(summary_fields[0]); ++i) {
        TempDir temp;
        ResultWriter writer(temp.path, [i] { return "summary-" + std::to_string(i); });
        assert(writer.create_run_directory().status == Status::Pass);
        SummaryRecord s = summary();
        s.*summary_fields[i] = std::numeric_limits<double>::quiet_NaN();
        assert(writer.write_summary(std::vector<SummaryRecord>(1, s)).status == Status::Fail);
    }
    TempDir temp;
    ResultWriter writer(temp.path, [] { return "raw-finite"; });
    assert(writer.create_run_directory().status == Status::Pass);
...
void test_injected_failures_and_indeterminate_publish() {
    for (const std::string failure : {"mkdir", "exists"}) {
        ...
        if (failure == "mkdir" || failure == "exists") {
            assert(result.status == Status::Fail);
            continue;
        }
        assert(result.status == Status::Pass);
        result = writer.write_environment(environment());
        assert(result.status == Status::Fail);
    }
    ...
    TempDir temp;
    std::shared_ptr<FakeFileOperations> ops(new FakeFileOperations());
    ops->publish_creates_final = true;
    ops->fail = "publish_remove";
    ResultWriter writer(temp.path, [] { return "rollback"; }, ops);
    assert(writer.create_run_directory().status == Status::Pass);
    CommandResult result = writer.write_environment(environment());
    assert(result.code == "OUTPUT_STATE_INDETERMINATE");
...
void test_real_concurrent_publish_is_complete_and_leaves_no_temp() {
    TempDir temp;
    ResultWriter writer(temp.path, [] { return "race"; });
    assert(writer.create_run_directory().status == Status::Pass);
    EnvironmentRecord a = environment(), b = environment();
    a.build_id = std::string(100000, 'A');
    b.build_id = std::string(100000, 'B');
    CommandResult ra, rb;
    std::thread ta([&] { ra = writer.write_environment(a); });
    std::thread tb([&] { rb = writer.write_environment(b); });
    ta.join();
    tb.join();
    assert((ra.status == Status::Pass) != (rb.status == Status::Pass));
    const std::string final = bytes(join(writer.run_directory(), "environment.tsv"));
    assert(final.find(std::string(100000, 'A')) != std::string::npos ||
           final.find(std::string(100000, 'B')) != std::string::npos);
...
void test_writer() {
    ...
    test_instant_ai_raw_schema_and_validation();
