...
AcquisitionData data(int points = 100, double duration = 1.0, double rate = 100.0)
{
    ...
    for (int i = 0; i < points; ++i) d.channels[0].samples.push_back(static_cast<double>(i));
    return d;
}
...
{
    FakeDaqAdapter a;
    a.enqueue_success(data()); a.enqueue_success(data()); a.enqueue_success(data());
...
{
    FakeDaqAdapter a;
    a.enqueue_success(data()); a.enqueue_success(data());
...
{
    FakeDaqAdapter short_read; short_read.enqueue_success(data(99));
    ...
    MatrixCase dual = base_case(); dual.reference_channel = MatrixValue<int>::from_value(1);
    AcquisitionData d = data(); d.channels.push_back(ChannelData()); d.channels[1].samples.resize(99);
    FakeDaqAdapter uneven; uneven.enqueue_success(d);
...
{
    FakeDaqAdapter exact; exact.enqueue_success(data(100, 1.01));
    assert(run_acquisition_case(exact, base_case()).status == Status::Pass);
    FakeDaqAdapter over; over.enqueue_success(data(100, 1.01001));
...
void test_driver_errors_and_unsupported_are_mapped()
{
    ...
    for (int i = 0; i < 3; ++i) {
        FakeDaqAdapter a; a.enqueue_failure(codes[i]);
    ...
    FakeDaqAdapter unsupported; unsupported.enqueue_unsupported("DUAL_CHANNEL_UNSUPPORTED");
...
{
    AcquisitionData timeout = data(); timeout.timed_out = true;
    AcquisitionData overrun = data(); overrun.overrun = true;
    AcquisitionData overflow = data(); overflow.cache_overflow = true;
    AcquisitionData values[] = {timeout, overrun, overflow};
    const char* codes[] = {"TIMEOUT", "OVERRUN", "CACHE_OVERFLOW"};
    for (int i = 0; i < 3; ++i) {
        FakeDaqAdapter a; a.enqueue_success(values[i]);
        CommandResult r = run_acquisition_case(a, base_case());
...
{
    FakeDaqAdapter mismatch; mismatch.enqueue_success(data(100, 1.0, 99.0));
    ...
    FakeDaqAdapter too_short; too_short.enqueue_success(data(100, 1.0));
    assert(run_acquisition_case(too_short, low).code == "WINDOW_TOO_SHORT");

    low.min_complete_cycles = MatrixValue<int>::from_value(2);
    AcquisitionData flat = data(); flat.channels[0].samples.assign(100, 1.0);
    FakeDaqAdapter span; span.enqueue_success(flat);
    assert(run_acquisition_case(span, low).code == "SIGNAL_SPAN_TOO_LOW");

    AcquisitionData valid = data();
    FakeDaqAdapter pass; pass.enqueue_success(valid);
...
void test_invalid_matrix_does_not_touch_hardware()
{
    ...
    MatrixCase required = base_case(); required.points_per_channel = MatrixValue<int>::required();
    r = run_acquisition_case(a, required);
    assert(r.exit_category == ExitCategory::InvalidArguments && a.configure_call_count() == 0);

    MatrixCase bad_mode = base_case(); bad_mode.mode = MatrixValue<std::string>::from_value("trigger");
    r = run_acquisition_case(a, bad_mode);
    assert(r.exit_category == ExitCategory::InvalidArguments && a.query_call_count() == 0);
    MatrixCase negative = base_case(); negative.sample_channel = MatrixValue<int>::from_value(-1);
    r = run_acquisition_case(a, negative);
    assert(r.exit_category == ExitCategory::InvalidArguments && a.query_call_count() == 0);
    MatrixCase bad_range = base_case(); bad_range.value_range = MatrixValue<std::string>::from_value("invalid");
    r = run_acquisition_case(a, bad_range);
    assert(r.exit_category == ExitCategory::InvalidArguments && a.query_call_count() == 0);
    MatrixCase incomplete_low = base_case(); incomplete_low.case_name = "low_sample_rate_100k";
...
{
    CapabilityInfo c; c.supports_acquisition = true; c.max_channels = 2;
    c.min_sample_rate_hz = 10; c.max_sample_rate_hz = 1000; c.max_points_per_channel = 1000;
    c.buffer_capacity = 2000; c.supported_ranges.push_back("-10V~10V"); return c;
}

void test_capabilities_are_enforced_before_configure()
{
    CapabilityInfo caps = capability(); caps.supports_acquisition = false;
    FakeDaqAdapter unsupported; unsupported.enqueue_query_success(caps);
    CommandResult r = run_acquisition_case(unsupported, base_case());
    assert(r.status == Status::Skip && unsupported.configure_call_count() == 0);

    MatrixCase dual = base_case(); dual.case_name = "dual_channel_reference";
    dual.reference_channel = MatrixValue<int>::from_value(1);
    dual.min_signal_span_v = MatrixValue<double>::from_value(1);
    CapabilityInfo one = capability(); one.max_channels = 1;
    FakeDaqAdapter channel; channel.enqueue_query_success(one);
    assert(run_acquisition_case(channel, dual).code == "CHANNEL_COUNT_UNSUPPORTED" && channel.configure_call_count() == 0);
    MatrixCase bad_index = base_case(); bad_index.sample_channel = MatrixValue<int>::from_value(2);
    FakeDaqAdapter index; index.enqueue_query_success(capability());
    assert(run_acquisition_case(index, bad_index).code == "CHANNEL_INDEX_UNSUPPORTED" && index.configure_call_count() == 0);

    CapabilityInfo low_rate = capability(); low_rate.min_sample_rate_hz = 101;
    FakeDaqAdapter rate; rate.enqueue_query_success(low_rate);
    assert(run_acquisition_case(rate, base_case()).code == "SAMPLE_RATE_UNSUPPORTED" && rate.configure_call_count() == 0);
    CapabilityInfo few_points = capability(); few_points.max_points_per_channel = 99;
    FakeDaqAdapter points; points.enqueue_query_success(few_points);
    assert(run_acquisition_case(points, base_case()).code == "POINT_COUNT_UNSUPPORTED" && points.configure_call_count() == 0);
    CapabilityInfo no_range = capability(); no_range.supported_ranges.clear(); no_range.supported_ranges.push_back("0V~10V");
    FakeDaqAdapter range; range.enqueue_query_success(no_range);
...
{
    FakeDaqAdapter query; query.enqueue_query_failure("QUERY_FAILED", "q", "query", "E1");
    ...
    FakeDaqAdapter configure; configure.enqueue_configure_unsupported("CONFIG_UNSUPPORTED", "c", "configure");
    ...
    FakeDaqAdapter trigger; trigger.enqueue_trigger_failure("TRIGGER_FAILED", "t", "trigger", "E2");
    TriggerRequest tr; tr.source = "external";
...
{
    FakeDaqAdapter a; a.enqueue_configure_success(); a.enqueue_configure_failure("CONFIGURE_SECOND");
    a.enqueue_configure_success(); a.enqueue_success(data()); a.enqueue_success(data());
    CommandResult r = run_acquisition_case(a, base_case(3));
...
{
    FakeDaqAdapter a; a.enqueue_failure("CODE,:|", "message,:|", "stage,:|");
    CommandResult r = run_acquisition_case(a, base_case());
    assert(r.code == "CODE,:|");
...
{
    AcquisitionData invalid_rate = data(); invalid_rate.actual_sample_rate_hz = std::numeric_limits<double>::quiet_NaN();
    FakeDaqAdapter rate; rate.enqueue_success(invalid_rate); CommandResult r = run_acquisition_case(rate, base_case());
    ...
    AcquisitionData infinite_rate = data(); infinite_rate.actual_sample_rate_hz = std::numeric_limits<double>::infinity();
    FakeDaqAdapter inf; inf.enqueue_success(infinite_rate);
    assert(run_acquisition_case(inf, base_case()).evidence.at("field") == "actual_sample_rate_hz");
    AcquisitionData negative_duration = data(); negative_duration.duration_seconds = -1.0;
    FakeDaqAdapter duration; duration.enqueue_success(negative_duration);
    assert(run_acquisition_case(duration, base_case()).evidence.at("field") == "duration_seconds");
    AcquisitionData bad_sample = data(); bad_sample.channels[0].samples[7] = std::numeric_limits<double>::quiet_NaN();
    FakeDaqAdapter sample; sample.enqueue_success(bad_sample); r = run_acquisition_case(sample, base_case());
...
public:
    ...
    AdapterResult<CapabilityInfo> query_capabilities(const std::string&) override {
        if (point_ == Query) throw std::runtime_error("query"); AdapterResult<CapabilityInfo> r; r.success=true; r.value=capability(); return r;
    }
    AdapterResult<OperationInfo> configure(const AcquisitionRequest&) override {
        if (point_ == Configure) throw std::runtime_error("configure"); AdapterResult<OperationInfo> r; r.success=true; return r;
    }
    AdapterResult<AcquisitionData> acquire_once(const AcquisitionRequest&) override {
        if (point_ == Acquire) throw std::runtime_error("acquire"); AdapterResult<AcquisitionData> r; r.success=true; r.value=data(); return r;
    }
    AdapterResult<OperationInfo> configure_trigger(const TriggerRequest&) override { return AdapterResult<OperationInfo>(); }
    void stop() noexcept override { ++stops_; }
    int stops() const { return stops_; }
private: Point point_; int stops_;
};
...
void test_driver_exceptions_are_caught_and_stopped()
{
    ...
    for (int i=0; i<3; ++i) {
        ThrowingAdapter a(points[i]); CommandResult r = run_acquisition_case(a, base_case(i == 0 ? 1 : 3));
        assert(r.code == "DRIVER_EXCEPTION" && r.exit_category == ExitCategory::Driver && a.stops() == (i == 0 ? 1 : 3));
...
{
    AcquisitionData separated=data();separated.trigger_wait_seconds=10;separated.wall_elapsed_seconds=11;AcquisitionRequest separated_request;separated_request.channels.push_back(0);separated_request.sample_rate_hz=100;separated_request.points_per_channel=100;assert(validate_acquisition_data(separated,separated_request).status==Status::Pass);
