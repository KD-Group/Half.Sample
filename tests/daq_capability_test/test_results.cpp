...
{
    assert(to_json(make_result(Status::Pass, ExitCategory::Success)).find(
        "\"result\":\"PASS\"") != std::string::npos);
    assert(to_json(make_result(Status::Fail, ExitCategory::ValidationFailed)).find(
        "\"result\":\"FAIL\"") != std::string::npos);
    assert(to_json(make_result(Status::Skip, ExitCategory::Unsupported)).find(
        "\"result\":\"SKIP\"") != std::string::npos);
}
