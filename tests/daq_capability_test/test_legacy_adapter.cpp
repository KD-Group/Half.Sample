...
void test_legacy_adapter()
{
    ...
    assert(daq_capability_test::select_legacy_supported_voltage_range(
    ...
    changed.timeout_seconds=4; assert(!daq_capability_test::same_acquisition_request(saved, changed));
    daq_capability_test::OnceInitializer<int> once; std::atomic<int> calls(0);
    std::thread a([&]{ once.get([&]{ ++calls; return 7; }); });
    std::thread b([&]{ once.get([&]{ ++calls; return 7; }); }); a.join(); b.join(); assert(calls==1);
    std::string mapped;
    ...
    const std::vector<std::vector<double> > output =
        daq_capability_test::deinterleave_legacy_samples(input, 2);
